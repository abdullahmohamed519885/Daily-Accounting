...
(() => {
  ...
  const launcherToggle = document.getElementById("installLauncherToggle");
  ...
  // Add a device class so Android gets a compact, non-blocking install banner.
  ...
  function applyLauncherCollapse() {
    if (!wrapper) return;
    wrapper.classList.toggle("is-collapsed", launcherCollapsed);
    launcherToggle?.setAttribute("aria-expanded", String(!launcherCollapsed));
    launcherToggle?.setAttribute("aria-label", launcherCollapsed ? "إظهار بطاقة التثبيت" : "طي بطاقة التثبيت");
    const icon = launcherToggle?.querySelector("i");
    if (icon) icon.className = launcherCollapsed ? "fa-solid fa-chevron-up" : "fa-solid fa-chevron-down";
  }
  ...
  function setInstallLauncher(state) {
    ...
    wrapper.classList.remove("is-visible", "is-hidden");
    ...
    if (state === "ready") {
      ...
      buttonSubtext.textContent = isAndroid ? "ثبّت التطبيق للوصول إليه بسرعة من جهازك" : "التثبيت متاح الآن — تجربة مثل تطبيق سطح المكتب";
      ...
      // Keep the launcher visible on mobile and on browsers that do not expose beforeinstallprompt.
      wrapper.classList.add("is-visible");
      button.classList.add("is-disabled");
      buttonText.textContent = "ثبّت أذكاري على جهازك";
      buttonSubtext.textContent = isAndroid ? "ثبّت التطبيق من Chrome أو أضفه إلى الشاشة الرئيسية" : "اضغط لمعرفة طريقة التثبيت المناسبة لمتصفحك";
      buttonArrow.className = "fa-solid fa-download";
      button.setAttribute("aria-label", "عرض خيارات تثبيت أذكاري");
    }
    applyLauncherCollapse();
  }
  ...
  // Always show the launcher by default. Mobile Safari/iOS and some Android browsers do not expose
  // beforeinstallprompt, so visibility must never depend on that event.
